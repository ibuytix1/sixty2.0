...
<script type="text/javascript">
	...
	$(document).ready(function(){
        ...
        var organizerTable = $('#organizer_list').DataTable({
			...
			},
			{
                ...
                "data": function (row, type, val, meta) {
            		...
            		action+='<li><a href="' + viewOrgnizerProfile + '/' + row[0] + '"><i class="fa fa-eye pr-2"></i> View Detail</a></li>
            		action+='<li><a href="' + editOrganizer + '/' + row[0] + '"><i class="fa fa-pencil pr-2"></i> Update Organizer</a></li>';
            		action+='<li><a href="javascript:;" class="delete_row" data-id="' + row[0] + '" data-type="Organizer" data-name="'+row[1]+'"><i class="fa fa-trash pr-2"></i> Delete</a></li></ul></div>';
            		return action;
                }
            },
			{
		...
		$('#modal-warning').on('click', '.deleteConfirm', function(){
			deletePopupSubmit( $(this), '{{ url("Admin/deleteOrganizer")}}', '{{ csrf_token() }}', organizerTable );
		});
